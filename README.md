...
| --add-metadata                    |                     | false    | Attempts to add a base level of episode metadata to each episode. Recommended only in cases where the original metadata is of poor quality. (**ffmpeg required**)                                                             |
...
| --audio-format                    | String              | false    | Convert audio to specified format. Supported formats: m4a, aac, mp3, opus, ogg, flac, wav. (**ffmpeg required**)                                                                                                              |
| --override                        |                     | false    | Override local files on collision.                                                                                                                                                                                            |
| --always-postprocess              |                     | false    | Always run additional tasks on the file regardless if the file already exists. This includes `--add-metadata`, `--adjust-bitrate`, `--mono`, `--audio-format`, and `--exec`.                                                  |
